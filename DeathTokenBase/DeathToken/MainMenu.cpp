...
MainMenu::MainMenu(Game* g) : GameState(g)
{
	...
	level1 = new Button(this, 200, 260, 175, 25, game->getTexture(MARIO), game->getTexture(MARIO));
	addObjects(level1);
	addEventListener(level1);
	level1->connect([this]() { lvl1(); });

	level2 = new Button(this, 200, 300, 175, 25, game->getTexture(MARIO), game->getTexture(MARIO));
	addObjects(level2);
	addEventListener(level2);
	level2->connect([this]() { lvl2(); });

	salir = new Button(this, 225, 340, 125, 25, game->getTexture(MARIO), game->getTexture(MARIO));
	addObjects(salir);
