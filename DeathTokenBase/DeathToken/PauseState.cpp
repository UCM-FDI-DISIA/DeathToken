...
PauseState::PauseState(Game* g) : GameState(g)
{
	...
	cont = new Button(this, 175, 150, 225, 25, game->getTexture(MARIO), game->getTexture(MARIO));
	addObjects(cont);
	addEventListener(cont);
	cont->connect([this]() { continuar(); });

	menu = new Button(this, 113, 250, 350, 25, game->getTexture(MARIO), game->getTexture(MARIO));
	addObjects(menu);
	addEventListener(menu);
	menu->connect([this]() { backToMenu(); });

	salir = new Button(this, 225, 350, 125, 25, game->getTexture(MARIO), game->getTexture(MARIO));
	addObjects(salir);
