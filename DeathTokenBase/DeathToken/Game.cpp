...
Game::Game()
{
  ...
  if (!soundManager.cargarSonido(
        "assets/sonido/Music/SlotsDT.wav", "SlotsM", SoundManager::MUSICA)) {
    std::cerr << "Error al cargar el sonido del Button." << std::endl;
  }
  if (!soundManager.cargarSonido(
        "assets/sonido/Music/BaccaratDToken.wav", "BaccaratM", SoundManager::MUSICA)) {
    std::cerr << "Error al cargar el sonido del Button." << std::endl;
  }
  if (!soundManager.cargarSonido(
        "assets/sonido/Music/PeleasDT.wav", "PeleasM", SoundManager::MUSICA)) {
    std::cerr << "Error al cargar el sonido del Button." << std::endl;
  }
  if (!soundManager.cargarSonido(
        "assets/sonido/Music/CanicasDT_V2.wav", "CanicasM", SoundManager::MUSICA)) {
    std::cerr << "Error al cargar el sonido del Button." << std::endl;
  }
  ...
  while (!empty()) {
    ...
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_QUIT ||
        ...
        FirebaseUtils::SaveState(PlayerEconomy::getBlueSouls(),
                                 PlayerEconomy::getRedSouls(),
                                 PlayerEconomy::getInsanity());
        stop();
      }
      else if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_p &&
               typeid(*gameStates.top()) != typeid(MainMenu) &&
               typeid(*gameStates.top()) != typeid(FinalMenu)) {
        if (!pause) {
