...
SlotsNormal::SlotsNormal(Game* g)
  : Slots(g)
  , comprobanteIndice(0)
{
  // Tamaños relativos al tamaño de pantalla
  float x =
    Game::WIN_WIDTH * (0.5f - ((TAM_CELDA - 50) / 1920.0f) * (N_COLUM / 2.0f));
  ...
  for (int i = 0; i < N_COLUM; ++i) {
    ...
    carretes.push_back(new Carrete(this,
                                   { (int)(x + i * celdaX), (int)y },
                                   (int)celdaX,
                                   (int)celdaY,
                                   game->getTexture(CELDA),
                                   game->getTexture(ICONOS)));
    addObjects(carretes[i]);

    // Botón correspondiente para detener el giro
    Button* button = new Button(this,
                                (int)(x + i * celdaX + (celdaX - botonX) / 2),
                                (int)(y * 2 + celdaY * N_COLUM),
                                (int)botonX,
                                (int)botonY,
                                game->getTexture(BTNSLOT),
                                game->getTexture(BTNSLOTCLICK));
    addObjects(button);
    ...
    button->connect([this, c] {
      ...
      if (c->getParada()) {
        ...
        if (comprobanteIndice == N_COLUM && canalSonidoGiro != -1) {
  ...
  x = Game::WIN_WIDTH *
      (0.5f - ((TAM_CELDA - 20) / 1920.0f) * ((N_COLUM / 2.0f) + 1.0f));
  y = Game::WIN_HEIGHT * 0.5f - h / 2;
  btnBet = new ButtonSlots(
    this, game, ui, int(x), (int)y, (int)w, (int)h, game->getTexture(BETSLOTS));
  addObjects(btnBet);
...
void SlotsNormal::update()
{
  ...
  // Si se han parado los tres carretes, se hace la comprobación
  if (comprobanteIndice == N_COLUM) {
    ...
    for (int i = 0; i < N_COLUM; ++i) {
      ...
      bool telarañas1_2 =
        vectorCarrete1[i] == vectorCarrete2[i] && vectorCarrete1[i] == 0;
      bool telarañas1_3 =
        vectorCarrete1[i] == vectorCarrete3[i] && vectorCarrete1[i] == 0;
      bool telarañas2_3 =
        vectorCarrete2[i] == vectorCarrete3[i] && vectorCarrete2[i] == 0;

      // Comprobante de los tres iconos en la fila iguales
      if (vectorCarrete1[i] == vectorCarrete2[i] &&
          vectorCarrete2[i] == vectorCarrete3[i]) {
